...
describe "After quiz fuction" do
  ...
  it "saves the students answers" do
    visit "/student/#{@student.slug}/quiz/#{@stquiz2.id}"
    choose("#{@question1.id}_ans_2")
    choose("#{@question2.id}_ans_1")
    choose("#{@question3.id}_ans_2")

    click_button "Submit"

    expect(StudentQuestion.count).to eq(3)
    expect(StudentQuestion.first.question).to eq(@question1)
    expect(StudentQuestion.first.student_answer).to eq("ans_2")
