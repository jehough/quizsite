...
describe "Student Quiz Controller" do
  ...
  @stquiz1 = StudentQuiz.create(:student_id => @student.id, :quiz_id => @quiz.id, :course_quiz => course_quiz.id, :open_time => Time.now + 3600, :close_time => (Time.now + 7200))
  @stquiz2 = StudentQuiz.create(:student_id => @student.id, :quiz_id => @quiz.id, :course_quiz => course_quiz.id, :open_time => Time.now - 7200, :close_time => (Time.now - 3600))
  before do
  ...
  it "will not access the quiz if not in right timeframe" do
    ...
    expect(last_response.body).to include("not open")
  end
